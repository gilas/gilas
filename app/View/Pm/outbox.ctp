        ...
        <td class="sender"><?php
            ...
            if(count($message['Recipients']) > 1){
                foreach($message['Recipients'] as $r){
                    $recipients .= $r['Recipient']['full_name'] . '، ';
                }
                if(strlen($recipients) > 20){
                    $recipients = substr($recipients,0,200) . ' ...';
                }
            }else{
                $recipients = $message['Recipients'][0]['Recipient']['full_name'];
            }
            echo $recipients .((isset($message['reply_count']))?" ({$message['reply_count']})":''); 
        ?></td>
