...
//echo $this->element('pm_search');
if(empty($messages)){
    echo 'هیچ پیامی دریافت نشده است';
    return;
}
?>
...
<table class="users pm-list">
    ...
    <tr <?php if($message['Reader']['new'] == 1) echo 'class="new-mail"'?> >
        ...
        App::import('Sanitize');
        $body_message = Sanitize::html($message['Message']['message'], array('remove' => true));
        $body_message = ' '. substr($body_message,0,50).' ... ';
        $body_message = $html->tag('span',$body_message,array('style' => 'color:gray !important'));
        echo $html->link($message['Message']['subject'].$body_message,array('action'=>'read',$message['Reader']['id']),array('escape' => false)); 
        ?></td>
