...
jQuery.fn.makeacolumnlists = function(settings){
	...
	if(jQuery('> li', this)) {
		this.each(function(y) {
			...
			jQuery(this).css({cssFloat:'right', width:''+col_Width+'px'});
			for (colnum=2; colnum<=settings.cols; colnum++) {
				if(jQuery(this).is('ol')) {
					jQuery('li.li_col'+ colnum, this).appendTo('#li_container' + y).wrapAll('<ol class="li_cont'+colnum +' ' + classN + '" style="float:right; width: '+col_Width+'px;"></ol>');
				} else {
					jQuery('li.li_col'+ colnum, this).appendTo('#li_container' + y).wrapAll('<ul class="li_cont'+colnum +' ' + classN + '" style="float:right; width: '+col_Width+'px;"></ul>');
				}
			}
